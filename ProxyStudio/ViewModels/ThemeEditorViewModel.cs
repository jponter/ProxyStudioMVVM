...
// REPLACE your GenerateFullThemeXamlFixed() method with this one
// This generates EXACTLY the same colors as your preview method

/// <summary>
/// ALIGNED: Generate theme XAML using EXACT same logic as preview
/// </summary>
private string GenerateFullThemeXamlFixed()
{
    ...
    sb.AppendLine($"  <!-- ALIGNED: Uses EXACT same logic as preview -->");
    sb.AppendLine();
    ...
    // Create a temporary resource dictionary to simulate what preview does
    var tempResources = new ResourceDictionary();
    
    // Apply colors using EXACT same method as preview
    ApplyColorsToResources(tempResources, FoundationColors, "Foundation");
    ApplyColorsToResources(tempResources, SemanticColors, "Semantic");
    ApplyColorsToResources(tempResources, SurfaceColors, "Surface");
    ApplyColorsToResources(tempResources, TextColors, "Text");
    
    // Apply derived colors using EXACT same method as preview
    ApplyDerivedColorsToResourcesForExport(tempResources);

    // Now output all the colors that the preview method generated
    sb.AppendLine("          <!-- ===== CORE COLORS (Generated by ApplyColorsToResources) ===== -->");
    OutputColorsFromResources(sb, tempResources, FoundationColors, "FOUNDATION");
    OutputColorsFromResources(sb, tempResources, SemanticColors, "SEMANTIC");  
    OutputColorsFromResources(sb, tempResources, SurfaceColors, "SURFACE");
    OutputColorsFromResources(sb, tempResources, TextColors, "TEXT");

    sb.AppendLine("          <!-- ===== DERIVED COLORS (Generated by ApplyDerivedColorsToResources) ===== -->");
    OutputDerivedColorsFromResources(sb, tempResources);
...
/// <summary>
/// Output colors from resource dictionary to XAML (preserves exact preview colors)
/// </summary>
private void OutputColorsFromResources(StringBuilder sb, IResourceDictionary resources, 
    ObservableCollection<ColorProperty> colorProperties, string sectionName)
{
    sb.AppendLine($"          <!-- ===== {sectionName} COLORS ===== -->");
    
    foreach (var colorProp in colorProperties)
    {
        var colorName = colorProp.Name.Replace(" ", "");
        var colorKey = colorName + "Color";
        var brushKey = colorName + "Brush";
        
        sb.AppendLine($"          <!-- {colorProp.Name}: {colorProp.Description} -->");
        
        // Output the color that was actually generated by ApplyColorsToResources
        if (resources.TryGetValue(colorKey, out var colorObj) && colorObj is Color color)
        {
            var colorHex = ColorToHex(color);
            sb.AppendLine($"          <Color x:Key=\"{colorKey}\">{colorHex}</Color>");
            sb.AppendLine($"          <SolidColorBrush x:Key=\"{brushKey}\" Color=\"{{DynamicResource {colorKey}}}\"/>");
        }
        else
        {
            // Fallback to original hex value
            sb.AppendLine($"          <Color x:Key=\"{colorKey}\">{colorProp.HexValue}</Color>");
            sb.AppendLine($"          <SolidColorBrush x:Key=\"{brushKey}\" Color=\"{{DynamicResource {colorKey}}}\"/>");
        }
...
/// <summary>
/// Output all derived colors that were generated by the preview method
/// </summary>
private void OutputDerivedColorsFromResources(StringBuilder sb, IResourceDictionary resources)
{
    var derivedKeys = resources.Keys.OfType<string>()
        .Where(k => k.Contains("Hover") || k.Contains("Light") || k.Contains("TextOn"))
        .OrderBy(k => k)
        .ToList();

    foreach (var key in derivedKeys)
    {
        if (resources.TryGetValue(key, out var resourceValue))
        {
            if (key.EndsWith("Color") && resourceValue is Color color)
            {
                var colorHex = ColorToHex(color);
                sb.AppendLine($"          <Color x:Key=\"{key}\">{colorHex}</Color>");
            }
            else if (key.EndsWith("Brush") && resourceValue is SolidColorBrush brush)
            {
                var colorKey = key.Replace("Brush", "Color");
                sb.AppendLine($"          <SolidColorBrush x:Key=\"{key}\" Color=\"{{DynamicResource {colorKey}}}\"/>");
            }
        }
    }
}
...
/// <summary>
/// Apply derived colors using EXACT same method as preview (for export alignment)
/// </summary>
private void ApplyDerivedColorsToResourcesForExport(IResourceDictionary resources)
{
    try
    {
        _logger.LogDebug("Generating derived colors for export using preview method...");

        // Use EXACT same logic as your preview method
        // Generate hover states for Foundation colors (15% darker)
        GenerateHoverState(resources, "Primary");
        GenerateHoverState(resources, "Secondary");
        GenerateHoverState(resources, "Surface");
        GenerateHoverState(resources, "Border");
        ...
        if (resources.TryGetValue("SurfaceColor", out var surfaceColorObj) && surfaceColorObj is Color surfaceColor)
        {
            var surfaceHover = DarkenColor(surfaceColor, 0.05f);
            resources["SurfaceHoverColor"] = surfaceHover;
            resources["SurfaceHoverBrush"] = new SolidColorBrush(surfaceHover);
        }

        _logger.LogDebug("Export derived colors generation completed (using preview method)");
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "Failed to generate derived colors for export");
    }
}
